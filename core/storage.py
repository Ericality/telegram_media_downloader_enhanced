...
async def record_failed_task(chat_id: Union[int, str], message_id: int, error_msg: str):
    ...
    try:
        failed_tasks_file = os.path.join(
            app.get_config("session_file_path"), FAILED_TASKS_FILE
        )
        tasks = []
        ...
        try:
            cfg = app.chat_download_config.get(chat_id) or app.chat_download_config.get(
                str(chat_id)
            )
            if cfg and cfg.node:
        ...
        if existing_index >= 0:
            ...
            logger.warning(
                f"更新失败任务: chat={chat_title}({chat_id}), message_id={message_id}, 重试次数: {retry_count}"
            )
        else:
            tasks.append(
                {
            ...
            retry_count = 0
            logger.warning(
                f"记录新失败任务: chat={chat_title}({chat_id}), message_id={message_id}"
            )
