...
def test_can_download_all():
    assert _can_download("video", {"video": ["all"]}, "mp4") is True


def test_can_download_matching_format():
    assert _can_download("video", {"video": ["mp4"]}, "mp4") is True


def test_can_download_unmatched_format():
    assert _can_download("video", {"video": ["mp4"]}, "avi") is False


def test_can_download_non_restricted_type():
    assert _can_download("photo", {"video": ["mp4"]}, None) is True


def test_is_exist_file(tmp_path):
    f = tmp_path / "x.mp4"
    f.write_text("x")
    assert _is_exist(str(f)) is True


def test_is_exist_directory(tmp_path):
    d = tmp_path / "sub"
    d.mkdir()
    assert _is_exist(str(d)) is False


def test_is_exist_missing(tmp_path):
    assert _is_exist(str(tmp_path / "nope.mp4")) is False


def test_check_timeout():
    assert _check_timeout(2, 0) is True
    assert _check_timeout(0, 0) is False
    assert _check_timeout(1, 0) is False
